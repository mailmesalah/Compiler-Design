...
class MultipleBoolTerms extends Expression
{
	...
	Val TypeEval(HashMap<String, Val> state) {
		...
		Val vBt=bt.TypeEval(state);
		Val vEx=ex.TypeEval(state);
	...
	Val Eval(HashMap<String, Val> state) {
		// TODO Auto-generated method stub
		Val vBp=bt.Eval(state);
		Val vEx=ex.Eval(state);
		
		if(vBp==null || vEx==null){
			return null;
		}
		
		if(vBp.getClass()==TypeBoolean.class && vEx.getClass()==TypeBoolean.class){
			return new TypeBoolean((((TypeBoolean)vBp).val || ((TypeBoolean)vEx).val));			
		}
		return null;
	}
}
